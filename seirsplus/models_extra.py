...
class VaccSEIRSNetworkModel():
    """
    A class to simulate the SEIRS Stochastic Network Model extended by the use of Vaccines
    ======================================================
    Params: 
            ...
            v               Probability of individual being administered a vaccine (optional: dictionary by node group)
            ve              Vaccine efficency percentage used to reduce transmission from infected individuals         
            ...
            initV_S         Initial number of vaccinated susceptible individuals
                            (all remaining nodes initialized susceptible)   

            f               What the hell is this supposed to be? Something about limiting the likelyhood of transitions to R or F states
            alpha           Non documented too. Looks like a factor to limit the rate of transmition to E and I states.
            alpha_Q         Alpha parameter but for Quarantined individuals
            delta           No clue whatsoever about this
    """
    def __init__(self, G, beta, sigma, gamma,
                    ...
                    initE=0, initI=0, initR=0, initF=0, initQ_E=0, initQ_I=0, initV_S=0,
        ...
        self.parameters = { 'G':G, 'G_Q':G_Q,
                            ...
                            'v': v, 've': ve, 'node_groups': node_groups,
                            'initV_S': initV_S }
        ...
        ### Pedro: Creo que lo que el muchacho quiere hacer aca, que es ultra manija en terminos de pedir memoria, es de por si establecer que la cantidad maxima de
        ### series tau que habran en la simulacion es equivalente al cardianal del conjunto de todos los eventos que pueden suceder a lo largo de la simulación SEIR.
        ### Por lo tanto instancia esto para llevar la cuenta de cuantos nodos hay en cada estado para cada iteracion.
        ### WARNING: modificar a 7 si vamos a agregar un nuevo estado, aunque hay en el codigo una funcion que permite resizear esta lista cuando el modelo se queda
        ### sin espacio
        ...
        self.numV_S     = numpy.zeros(6*self.numNodes)
        ...
        self.numV_S[0]      = int(initV_S)
        self.numS[0]        = (self.numNodes - self.numE[0] - self.numI[0] - self.numR[0] - self.numQ_E[0] - self.numQ_I[0] - self.numF[0] - self.numV_S[0])
        ...
        self.X = numpy.array( [self.S]*int(self.numS[0]) + [self.E]*int(self.numE[0]) + [self.I]*int(self.numI[0]) 
                               ...
                               + [self.Q_E]*int(self.numQ_E[0]) + [self.Q_I]*int(self.numQ_I[0]) + [self.V_S]*int(self.numV_S[0])
        ...
        self.transitions =  { 
                                ...
                                'StoVS': {'currentState':self.S, 'newState': self.V_S},
                                'VStoE': {'currentState':self.V_S, 'newState': self.E}
                            }
        ...
        self.vaccinated  = numpy.array([False]*self.numNodes).reshape((self.numNodes,1))
        self.positive    = numpy.array([False]*self.numNodes).reshape((self.numNodes,1))
        self.contracted  = numpy.array([False]*self.numNodes).reshape((self.numNodes,1))
        self.numTested   = numpy.zeros(6*self.numNodes) 
        self.numPositive = numpy.zeros(6*self.numNodes) 
        self.numCases    = numpy.zeros(6*self.numNodes) 
        self.numVaccinated = numpy.zeros(6*self.numNodes) 
        ...
        if(node_groups):
            ...
            for groupName, nodeList in node_groups.items():
                ...
                self.nodeGroupData[groupName]['numV_S']         = numpy.zeros(6*self.numNodes)
                ...
                self.nodeGroupData[groupName]['numCases']       = numpy.zeros(6*self.numNodes)
                self.nodeGroupData[groupName]['numTested']      = numpy.zeros(6*self.numNodes)
                self.nodeGroupData[groupName]['numVaccinated']  = numpy.zeros(6*self.numNodes)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def update_parameters(self):
        ...
        #----------------------------------------
        # Add v array so it's defined by the node group each node is part of
        self.v              = ((numpy.array(self.parameters['v']).reshape((self.numNodes, 1))       if isinstance(self.parameters['v'], (list, numpy.ndarray))  else numpy.full(fill_value=self.parameters['v'], shape=(self.numNodes,1)))  if not isinstance(self.parameters['v'], Mapping) else self.fill_v_by_node_group(self.parameters['v'], self.parameters['node_groups'])) if self.parameters['v'] is not None else self.v
        self.ve             = (numpy.array(self.parameters['ve']).reshape((self.numNodes, 1))       if isinstance(self.parameters['ve'], (list, numpy.ndarray)) else numpy.full(fill_value=self.parameters['ve'], shape=(self.numNodes,1))) if self.parameters['ve'] is not None else self.ve
        ...
        #----------------------------------------
        ### Pedro: Parametro para establecer las probabilidades de infeccion globales, para los casos de exposicion via el caso del modelo SEIR base, y no tomando en cuenta la red
        if(self.beta_pairwise_mode == 'infected' or self.beta_pairwise_mode is None):
            ...
            ### Pedro: Interesante pero, ni idea que uso podria llegar a tener este modo
            ...
            ### Pedro: wut? Sumale el promedio y dividilo por 2? Es algun tipo de normalizacion esto?
        ...
        #----------------------------------------
        ### Pedro: Beta local es un beta definido para casos donde haya un arista que una a dos nodos. I guess...
        ### Que droga, te permite definir betas para literalmente cada par de nodos.
        ...
        #----------------------------------------
        ### Pedro: si es una matriz dejalo como esta
        if(self.beta_local.ndim == 2 and self.beta_local.shape[0] == self.numNodes and self.beta_local.shape[1] == self.numNodes):
            ...
            ### Pedro: Aplanar la matriz a un arreglo? Supongo que eso implica que en el caso de que haya un beta_local de ndim == 2 es porque es de 1 fila/columna
            ...
            ### Pedro: Piola, esto es efectivamente sumar los beta de cada conexion entre un nodo y sus vecinos. La magia de las transposiciones no la entiendo pero confio en vos Ryan <3
            ### But still, asi no es como funciona la probabilidad no? Tendria que promediarlo por degree? Creo que hace eso despues!
            ### Edit: esto no deberia devolver una matriz de numNodes*numNodes??
            ### A no es simetrica ademas? Cual es el punto de multiplicar por T, no deberian ser exactamente iguales los resultados? Ryan, no confio tanto en vos ahora :c
            ### Edit 2.0: Ok, parece ser que hay una convencion implicita (python como te odio), donde si A no es una matriz numNodes*numNodes, entonces los deltas a su vez,
            ### tampoco vienen en forma de una matriz, cosa de que la multiplicaciones entre ambos A_beta y A_delta resuelva bien. Ryan, perdona por haber dudado de vos <3
        ...
        ### Pedro: 100% perdido en esta mostro
        ### Parece que delta llega como una matriz de valores que no sabemos bien que representa, pero en el caso de no estar definida (y asi es como ocurre por defaut)
        ### entonces se usa una lista de log(degree) en base mean(degree). En palabras la proporcion logaritmica entre ambos valores. Considerando nuestro grafo no creo que sea algo mayor a 2 con toda la furia
        ...
        #----------------------------------------
        ### Pedro: Multiplicar las matrices de deltas (se, yo tampoco se que representa) por la matriz de betas por cada conexion entre nodo
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def fill_v_by_node_group(self, v_map, node_groups):
        if node_groups is None or v_map is None:
            print("Empty node group data. Can't set vaccination by node group if no node group was originally set beforehand")
        # Iterate through node groups and set each node in V
        for groupName, nodeList in node_groups.items():
            for index in nodeList:
                self.v[index] = v_map[groupName]['v_prob']
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def total_num_vaccinated(self, t_idx=None):
        if(t_idx is None):
            return (self.numVaccinated[:])            
        else:
            return (self.numVaccinated[t_idx])

#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

    def total_num_vaccinated_susceptible(self, t_idx=None):
        if(t_idx is None):
            return (self.numV_S[:])            
        else:
            return (self.numV_S[t_idx])
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def calc_propensities(self):
        ...
        ### Pedro: ok, aca hay una magia precalculada de un arreglo donde por cada nodo tiene precomputado el aporte de infeccion que cada nodo infectado genera?
        ...
        #------------------------------------
        ### Maybe we should recompute this array of random values for each calculation of propensities. 
        ### Atually no. I finally know what this uncommented value is for. It pre sets which nodes will either die or recover from the sickness. The mu and sigma values
        ### are only to define the rate, but f (the fatality percentage I guess) is something that we precalculate for every node. WE SHOULD DEFINE THIS BY GROUP SO THE SIMULATIONS ARE COOLER!
        
        # Commented because it's wrong but I shouldn't forget what this CLEARLY DESCRIBED value is actually for
        # self.rand_f = numpy.random.rand(self.f.shape[0], self.f.shape[1])
        #------------------------------------
        ...
        ### Pedro: Obtene una lista que por cada indice/nodo resuelva cuantas personas estuvo en contacto, que dieron positivo para la enfermedad y que no se hayan recuperado aun
        ### Nah que ver. Anda a saber porque self.positive representa el conjunto de nodos que hicieron cuarentena. No lo usa para casos donde se hayan infectado
        ### SELF POSITIVE ES EL LISTADO DE NODOS QUE SE HICIERON UN TEST Y LES DIO POSITIVO. DICHO DE OTRA MANERA, NODOS INFECTADOS O EXPUESTOS DE LOS QUE SE SABE QUE ESTAN EN ESE ESTADO
        ...
        propensities_toE        = (self.alpha *
                                  ...
                                  ) * (numpy.array(self.X==self.S) | numpy.array(self.X==self.V_S))

        propensities_StoE       = propensities_toE * (self.X==self.S)

        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

        propensities_VStoE      = propensities_toE * (numpy.full(fill_value=1, shape=(self.numNodes,1)) - self.ve) * (self.X==self.V_S)
        ...
        if(self.transition_mode == 'time_in_state'):
            ...
            propensities_StoVS    = numpy.zeros_like(propensities_StoE)    
        ...
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            propensities_StoVS       = self.v * (self.X==self.S)   
        ...
        propensities = numpy.hstack([propensities_StoE, propensities_EtoI, 
                                     ...
                                     propensities_StoVS, propensities_VStoE])

        columns = ['StoE', 'EtoI', 'ItoR', 'ItoF', 'EtoQE', 'ItoQI', 'QEtoQI', 'QItoR', 'QItoF', 'RtoS', '_toS', 'StoVS', 'VStoE']
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def set_contracted(self, node, contracted):
        self.contracted[node] = contracted

#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

    def set_vaccinated(self, node, vaccinated):
        self.vaccinated[node] = vaccinated
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^   
    ...
    def increase_data_series_length(self):
        ...
        self.numCases      = numpy.pad(self.numCases, [(0, 6*self.numNodes)], mode='constant', constant_values=0)
        self.numVaccinated = numpy.pad(self.numVaccinated, [(0, 6*self.numNodes)], mode='constant', constant_values=0)
        ...
        if(self.nodeGroupData):
            for groupName in self.nodeGroupData:
                ...
                self.nodeGroupData[groupName]['numV_S']        = numpy.pad(self.nodeGroupData[groupName]['numV_S'], [(0, 6*self.numNodes)], mode='constant', constant_values=0)
                ...
                self.nodeGroupData[groupName]['numCases']      = numpy.pad(self.nodeGroupData[groupName]['numCases'], [(0, 6*self.numNodes)], mode='constant', constant_values=0)
                self.nodeGroupData[groupName]['numVaccinated'] = numpy.pad(self.nodeGroupData[groupName]['numVaccinated'], [(0, 6*self.numNodes)], mode='constant', constant_values=0)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ 
    ...
    def finalize_data_series(self):
        ...
        self.numCases      = numpy.array(self.numCases, dtype=float)[:self.tidx+1]
        self.numVaccinated = numpy.array(self.numVaccinated, dtype=float)[:self.tidx+1]
        ...
        if(self.nodeGroupData):
            for groupName in self.nodeGroupData:
                ...
                self.nodeGroupData[groupName]['numV_S']        = numpy.array(self.nodeGroupData[groupName]['numV_S'], dtype=float)[:self.tidx+1]
                ...
                self.nodeGroupData[groupName]['numCases']      = numpy.array(self.nodeGroupData[groupName]['numCases'], dtype=float)[:self.tidx+1]
                self.nodeGroupData[groupName]['numVaccinated'] = numpy.array(self.nodeGroupData[groupName]['numVaccinated'], dtype=float)[:self.tidx+1]
        return None
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^     
    ...
    def run_iteration(self):
        ...
        if(propensities.sum() > 0):
            ...
            if(transitionType in ['StoE', 'VStoE']):
                self.set_contracted(node=transitionNode, contracted=True)
                ...
                self.infectionsLog.append({ 't':                            self.t,
                                            ...
                                            'vaccinated': transitionType == 'VStoE' })
            ...
            if(transitionType in ['StoVS']):
                self.set_vaccinated(node=transitionNode, vaccinated=True)

            # Is there anything to actually measure with this transition? Vaccinations that were unsuccesful?
            # if(transitionType in ['VStoE']):
            #    self.set_vaccinated(node=transitionNode, vaccinated=True)
        ...
        self.numV_S[self.tidx]      = numpy.clip(numpy.count_nonzero(self.X==self.V_S), a_min=0, a_max=self.numNodes)
        ...
        self.numCases[self.tidx]    = numpy.clip(numpy.count_nonzero(self.contracted), a_min=0, a_max=self.numNodes)
        self.numVaccinated[self.tidx] = numpy.clip(numpy.count_nonzero(self.vaccinated), a_min=0, a_max=self.numNodes)
        ...
        if(self.nodeGroupData):
            for groupName in self.nodeGroupData:
                ...
                self.nodeGroupData[groupName]['numCases'][self.tidx]    = numpy.count_nonzero(self.nodeGroupData[groupName]['mask']*self.contracted)
                self.nodeGroupData[groupName]['numVaccinated'][self.tidx] = numpy.count_nonzero(self.nodeGroupData[groupName]['mask']*self.vaccinated)
            ...
            if(print_interval):
                if(print_reset and (int(self.t) % print_interval == 0)):
                    ...
                    if(verbose==True):
                        ...
                        print("\t V_S  = " + str(self.numV_S[self.tidx]))
                        print("\t numV  = " + str(self.numVaccinated[self.tidx]))
                    print_reset = False
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def plot(self, ax=None,  plot_S='line', plot_E='line', plot_I='line',plot_R='line', plot_F='line', plot_V='line',
                            plot_Q_E='line', plot_Q_I='line', combine_D=True,
                            color_S='tab:green', color_E='orange', color_I='crimson', color_R='tab:blue', color_V='tab:purple', color_F='black',
        ...
        Vseries     = self.numVaccinated/self.numNodes if plot_percentages else self.numVaccinated 

        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        # Draw the reference data:      
        #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

        # Pedro: no sabemos que es esto pero por el momento no lo hemos necesitado y se uso el default de None
        ...
        if(any(Vseries) and plot_V=='stacked'):
            ax.fill_between(numpy.ma.masked_where(Vseries<=0, self.tseries), numpy.ma.masked_where(Vseries<=0, topstack+Vseries), topstack, color=color_V, alpha=0.5, label='$V$', zorder=2)
            ax.plot(        numpy.ma.masked_where(Vseries<=0, self.tseries), numpy.ma.masked_where(Vseries<=0, topstack+Vseries),           color=color_V, zorder=3)
            topstack = topstack+Vseries
        ...
        if(any(Vseries) and plot_V=='shaded'):
            ax.fill_between(numpy.ma.masked_where(Vseries<=0, self.tseries), numpy.ma.masked_where(Vseries<=0, Vseries), 0, color=color_V, alpha=0.5, label='$V$', zorder=4)
            ax.plot(        numpy.ma.masked_where(Vseries<=0, self.tseries), numpy.ma.masked_where(Vseries<=0, Vseries),    color=color_V, zorder=5)
        ...
        if(any(Vseries) and plot_V=='line'):
            ax.plot(numpy.ma.masked_where(Vseries<=0, self.tseries), numpy.ma.masked_where(Vseries<=0, Vseries), color=color_V, label='$V$', zorder=6)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def figure_infections(self, plot_S=False, plot_E='stacked', plot_I='stacked',plot_R=False, plot_F=False, plot_V=False,
        ...
        self.plot(ax=ax, plot_S=plot_S, plot_E=plot_E, plot_I=plot_I,plot_R=plot_R, plot_F=plot_F,plot_V=plot_V,
                        plot_Q_E=plot_Q_E, plot_Q_I=plot_Q_I, combine_D=combine_D,
                        color_S=color_S, color_E=color_E, color_I=color_I, color_R=color_R, color_V=color_V, color_F=color_F,
                        color_Q_E=color_Q_E, color_Q_I=color_Q_I, color_reference=color_reference,
                        dashed_reference_results=dashed_reference_results, dashed_reference_label=dashed_reference_label, 
                        shaded_reference_results=shaded_reference_results, shaded_reference_label=shaded_reference_label, 
                        vlines=vlines, vline_colors=vline_colors, vline_styles=vline_styles, vline_labels=vline_labels, 
